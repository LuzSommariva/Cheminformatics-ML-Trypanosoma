...
print(f"ECFP Fingerprints shape: {X_ecfp_short.shape}")
print(f"ECFP Fingerprints  example values: {X_ecfp_short[0, :10]}")

target_df = data.reset_index(drop=True)

fingerprint_df = pd.DataFrame(X_ecfp_short).reset_index(drop=True)
...
output_dir = "fingerprints_ECFP"
os.makedirs(output_dir, exist_ok=True)

# Guardar el archivo con nombre único basado en el target
output_path = os.path.join(output_dir, f"fingerprints_ECFP_{base_filename}.csv")
