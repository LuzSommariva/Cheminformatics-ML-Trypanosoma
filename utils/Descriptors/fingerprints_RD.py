...
fingerprint_names = fp_rdkit_2d.get_feature_names_out()
fingerprint_df = pd.DataFrame(X_rdkit_2d, columns=fingerprint_names).reset_index(drop=True)
...
output_dir = "fingerprints_rdkit"
os.makedirs(output_dir, exist_ok=True)

# Guardar el archivo con nombre único basado en el target
output_path = os.path.join(output_dir, f"fingerprints_{base_filename}.csv")
