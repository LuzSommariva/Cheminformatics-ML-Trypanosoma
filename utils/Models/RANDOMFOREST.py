...
for target in targets:
   ...
   # Evaluar en el conjunto de prueba
    ...
    roc_data.to_csv(f'rocdata_RF/roc_data_RF_fingerprints_ECFP_{target}.csv', index=False)
    print(f'ROC data for target {target} saved as roc_data_RF_fingerprints_ECFP_{target}.csv')
    ...
    # Convertir a array numpy por si no lo están
    feature_importances = np.array(feature_importances)

    # Ordenar y seleccionar las top 30
    top_n = 20
    indices_ordenados = np.argsort(feature_importances)[::-1]
    top_indices = indices_ordenados[:top_n]
    top_importances = feature_importances[top_indices]

    # Gráfico
    plt.figure(figsize=(10, 6))
    plt.bar(range(top_n), top_importances, color="navy")
    plt.xticks(range(top_n), top_indices, rotation=0)  # Mostrar solo los índices
    plt.xlabel("Índice de característica")
    plt.ylabel("Importancia")
    plt.title(f'Feature Importances (Top {top_n}) for {target}')
    plt.tight_layout()

    # Guardar gráfico
    plt.savefig(f'feature_importances_plots_RF/{target}_feature_importances_fingerprints_ECFP_{top_n}.pdf')
    plt.close()
    ...
    resultados_path = 'resultados_RF/resultados_modelos_RF_fingerprints_ECFP.csv'

    # Si el archivo ya existe, lo cargamos y agregamos los nuevos resultados
    ...
    model_filename = f'modelos_RF/model_RF_fingerprints_ECFP_{target}.pkl'
